...
class account_invoice(models.Model):
    ...
    def afip_validation(self):
        ...
        for invoice in self:
            ...
            invoice_class = journal_class.document_class_id
            my_company = invoice.journal_id.company_id
            resp_class = self.env['afip.responsability_relation'].search([
                ('document_class_id', '=', invoice_class.id),
                ('issuer_id.code', '=',
                 my_company.partner_id.responsability_id.code)
            ])
            ...
            resp_class = self.env['afip.responsability_relation'].search([
                ('document_class_id', '=', invoice_class.id),
                ('receptor_id.code', '=',
                 invoice.partner_id.responsability_id.code)
            ])
            if not resp_class:
                raise except_orm(
                    _('Invalid receptor'),
